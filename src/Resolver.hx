...
class Resolver {
	...
	public function check( file : String ) {
		...
		for( f in platform.VFileSystem.readDirectory(file.substr(0, -basePath.length)) ) {
		...
		inline function parse(file) {
			return haxe.Json.parse(platform.VFile.getContent(file));
		}
		var merged = platform.VFile.getContent(file).split("<<<<<<< .mine");
		...
		try {
			platform.VFile.saveContent(platform.VSys.getEnv("TEMP") + "/" + basePath + ".merged" + minRev + "_" + maxRev, platform.VFile.getContent(file));
		} catch( e : Dynamic ) {
		}
		platform.VFile.saveContent(file, haxe.Json.stringify(other, null, "\t"));
		platform.VFileSystem.deleteFile(file+".mine");
		platform.VFileSystem.deleteFile(file+".r"+minRev);
		platform.VFileSystem.deleteFile(file+".r"+maxRev);
		return true;
	}
	...
	function resolveRec( mine : Dynamic, origin : Dynamic, other : Dynamic, path : Array<String> ) {
		...
		if( Std.is(mine, Array) ) {
			...
			if( Std.is(mine, String) && Std.is(other, String) ) {
				try {
					var dorigin = util.Lz4Reader.decodeString(origin);
					var dmine = util.Lz4Reader.decodeString(mine);
					var dother = util.Lz4Reader.decodeString(other);
					...
					// merged
					return util.Lz4Reader.encodeBytes(dother, other.substr(0, 5) == "BCJNG");
				} catch( e : Dynamic ) {
