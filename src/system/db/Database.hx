...
import data.Data.CustomType;
import data.Data.CustomTypeCase;
import data.Types;
...
class Database {
			...
			for( c in s.getColumns() )
				...
				case TLayer(_):
					for( obj in s.getScopedLines() ) {
						...
						ldat = Layer.encode(d, data.compress);
				...
				case TTileLayer:
					for( obj in s.getScopedLines() ) {
						...
						Reflect.setField(ldat,"data", TileLayerData.encode(d, data.compress));
					}
				default:
				}
		return b;
	...
	function addSheet( s : data.Data.SheetData, ?index : Int ) : Sheet {
	...
	public function load( content : String ) {
		var data = Parser.parse(content, true);
		loadData(data);
	}

	public function loadData( data : data.Data ) {
		this.data = data;
		if( sheets != null ) {
		...
		sheets = [for( s in data.sheets ) {
			if(s.sheetType == null){
				if(s.props.level != null) {
					s.sheetType == SheetTypes.LEVELS;
				}
				else{
					s.sheetType == SheetTypes.DATA_TABLE;
				}
			}
			new Sheet(this, s);
		}];
