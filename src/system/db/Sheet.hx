...
typedef SheetIndex = { id : String, disp : String, ico : data.Types.TilePos, obj : Dynamic }
...
class Sheet {
	...
	public function updateLine(index : Int, toCopy: Dynamic) {
		var line = sheet.lines[index];
		for( c in sheet.columns ) {
			Reflect.setField(line, c.name, Reflect.field(toCopy, c.name));
		}
	}
			...
			for( c in s.columns )
				...
				case TLayer(t) if( t == sheet.name ):
					for( obj in s.getLines() ) {
						...
						ldat = data.Types.Layer.encode(d, base.compress);
	...
	public function updateValue( c : Column, index : Int, old : Dynamic ) {
		...
		case TInt if( isLevel() && (c.name == "width" || c.name == "height") ):
			...
			function remapTileLayer( v : data.Types.TileLayer ) {
				...
				return { file : v.file, size : v.size, stride : v.stride, data : data.Types.TileLayerData.encode(ndat, base.compress) };
			}
			...
			for( c in sheet.columns ) {
				...
				case TLayer(_):
					...
					v = data.Types.Layer.encode(ndat, base.compress);
				...
				case TList:
					...
					if( s.hasColumn("x", [TInt,TFloat]) && s.hasColumn("y", [TInt,TFloat]) ) {
						...
						var a : Array<{ data : data.Types.TileLayer }> = v;
						for( o in a )
