 ...
 */
 ...
 //import plugin.kinds.Script_LevelObjectPlugin.LevelObjectScriptUtil;
 ...
 class Level {
	 ...
	 static var loadedTilesCache = new Map< String, { pending : Array < Int->Int->Array<util.render.Image>->Array<Bool>->Void >, data : { w : Int, h : Int, img : Array<util.render.Image>, blanks : Array<Bool> }} >();
	 ...
	 function watchSplit(key:String) {
		 ...
		 watch(file, function() util.render.Image.load(abs, function(_) { loadedTilesCache.remove(key); reload(); } , function() {
			 for( w in watchList )
				 if( w.path == abs )
					 w.time = 0;
		 }, true));
	 ...
	 public function loadAndSplit(file, size:Int, callb) {
		 ...
		 if( a == null ) {
			 ...
			 util.render.Image.load(model.getAbsPath(file), function(i) {
				 var images = [], blanks = [];
	 ...
	 function getFileTime(path) {
		 return 0.0; //try platform.VFileSystem.stat(path).mtime.getTime()*1. catch( e : Dynamic ) 0.;
	 }
	 ...
	 function hasHole( i : Image, x : Int, y : Int ) {
		 for( dx in -1...2 )
			 for( dy in -1...2 ) {
				 var x = x + dx, y = y + dy;
	 ...
	 @:keep function addNewLayer( name ) {
		 ...
		 case TList:
			 ...
			 var a : Array<{ name : String, data : data.Types.TileLayer }> = Reflect.field(obj, newLayer.name);
			 o.name = name;
			 a.push(o);
			 var n = a.length - 2;
			 while( n >= 0 ) {
				 var o2 = a[n--];
				 if( o2.data != null ) {
					 var a = data.Types.TileLayerData.encode([for( k in 0...width * height ) 0], model.compressionEnabled());
	 ...
	 function setup() {
		 ...
		 view = util.render.Image3D.getInstance();
		 ...
		 cursorImage = new util.render.Image(0, 0);
		 cursorImage.smooth = false;
		 tmpImage = new util.render.Image(0, 0);
		 ...
		 scroll.mousedown(function(e) {
			 ...
			 case 3:
				 trace("right click: " + selection.x + " " + selection.y + " " + selection.w + " " + selection.h + " " + e.ctrlKey);
				 if( selection != null ) {
					 ...
					 if( e.ctrlKey ) {
						 ...
						 trace("curPos: " + curPos.x + " " + curPos.y);
			 ...
			 if( palette.randomMode ) {
				 ...
				 for(cX in 0...Std.int(selection.w)){
					 for(cY in 0...Std.int(selection.h)){
						 var coordP = Std.int(selection.x + cX + (selection.y + cY) * l.stride);
						 var p = (x + cX) + (y + cY) * width;
						 var id = data[coordP];
						 if(l.blanks[id - 1]) continue;
						 data[p] = id;
						 changed = true;
					 }
				 }
