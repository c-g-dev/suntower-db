...
enum LayerInnerData {
	Layer(a:Array<Int>);
	Objects(idCol:String, objs:Array<{
		x:Float,
		y:Float,
		?width:Float,
		?height:Float
...
class LayerData extends LayerGfx {
	...
	public function setLayerData(val:String) {
		if (val == null || val == "")
			...
			var a = util.Lz4Reader.decodeString(val);
			if (a.length != level.width * level.height)
	...
	public function getSelObjects() {
		...
		for (o in tileProps.sets)
			if (o.t == Object
				&& !(o.x >= x + currentSelectionWidth || o.y >= y + currentSelectionHeight || o.x + o.w <= x || o.y + o.h <= y))
				out.push(o);
		return out;
	}
	...
	public function setTilesData(val:data.Types.TileLayer) {
		...
		if (file == null) {
			if (props.mode != Tiles && props.mode != null)
				Reflect.deleteField(props, "mode");
	...
	function set_visible(v) {
		visible = v;
		if (comp != null)
			comp.toggleClass("hidden", !visible);
		return v;
	}
	...
	function getData():Dynamic {
		switch (data) {
			case Layer(data):
				...
				return util.Lz4Reader.encodeBytes(b, level.model.compressionEnabled());
			case Objects(_, objs):
				return objs;
			case Tiles(t, data):
				...
				return t.file == null ? null : {
					file: t.file,
					size: t.size,
					stride: t.stride,
					data: util.Lz4Reader.encodeBytes(b.getBytes(), level.model.compressionEnabled())
				};
	...
	public inline function initMatrix(m:{
		a:Float,
		b:Float,
		c:Float,
		d:Float,
	...
	public function draw(view:Image3D) {
		...
		switch (data) {
			...
			case Tiles(t, data):
				...
				if (props.mode == Ground) {
					var b = new util.render.TileBuilder(tileProps, stride, images.length);
					...
					if (objs != null) {
						if (idCol == null) {
							for (o in objs) {
								...
								LevelObjectDisplayUtil.apply(LevelObjectPluginContext.createForLevelObjectDisplay(this, view, Std.int(o.x * size),
									Std.int(o.y * size), o));
