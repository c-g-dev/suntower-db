...
class Palette {
	...
	var perTileGfx : Map<String, LayerGfx>;
	...
	public function init() {
		...
		for( c in perTileProps )
			switch( c.type ) {
			case TRef(s):
				var g = new system.lvl.LayerGfx(level);
	...
	public function ensureBarIsOnScreen() {
		Timer.delay(() -> {
			...
			var ww = platform.Window.get().width;
			var wh = platform.Window.get().height;
			var bOff = bar.offset();
			if( bOff.left < 0 ) {
				paletteJ.css("left", 0);
			}
	...
	public function layerChanged( l : LayerData, ?resetScale: Bool = true ) {
		...
		var i = util.render.Image.fromCanvas(cast p.find("canvas.view")[0]);
		
		// Determine max width and calculate palette width
		var maxWidth = platform.Window.get().width / 2;
	...
	//		palWidth = l.stride * level.tileSize * scale;
		...
		var ipreview = util.render.Image.fromCanvas(cast jpreview.find("canvas")[0]);
		select = util.render.Image.fromCanvas(cast jsel[0]);
