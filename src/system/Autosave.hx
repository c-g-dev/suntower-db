...
class AutosaveEngine {
    ...
    */  
    ...
    public function save(path: String) {
        ...
        if (!platform.VFileSystem.exists(dir)) {
            platform.VFileSystem.createDirectory(dir);
        }
    ...
    function cleanupOldBackups(fileName: String, date: String) {
        ...
        var files = platform.VFileSystem.readDirectory(dir);
        ...
        if (backups.length > maxNum) {
            ...
            for (i in 0...backups.length - maxNum) {
                platform.VFileSystem.deleteFile(dir + "/" + backups[i]);
            }
        }
    }
}