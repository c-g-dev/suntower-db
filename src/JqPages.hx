 ...
 */
 ...
 class JqPage extends util.vdom.Server {
	 ...
	 override function send( msg : util.vdom.Answer ) {
		 /*var bytes = encodeAnswer(msg);
		 var buf = new Buffer(bytes.length + 2);
	 ...
	 override function handleSpecial( e : js.html.Element, name : String, args : Array<Dynamic>, result : Dynamic -> Void ) {
			 ...
			 fs.change(function(_) {
				 ...
				 if( saveAs ) {
					 /*if( Std.is(data, haxe.io.Bytes) ) {
						 ...
						 platform.VFile.saveContent(path, data);*/
				 }
		 ...
		 case "popupMenu":
			 ...
			 var n = new platform.Menu();
			 for( i in 0...args.length ) {
				 var mit = new platform.MenuItem( { label : args[i] } );
				 n.append(mit);
